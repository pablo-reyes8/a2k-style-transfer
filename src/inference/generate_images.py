...
def save_random_horizontal_sample(
    ...
    """
    Toma un par aleatorio del iterador, lo estiliza y guarda un grid horizontal
    (content | style | output) en out_path.
    """
    ...
    if b == 0:
        raise ValueError("Iterator yielded empty batches.")
    ...
    # desnormalizar content/style (ImageNet) y clipear output
    c0 = denorm_imagenet(x_c_sel[0]).cpu()
    s0 = denorm_imagenet(x_s_sel[0]).cpu()
    y0 = y[0].detach().cpu().float().clamp(0.0, 1.0)
